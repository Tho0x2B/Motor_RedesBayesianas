...
public class Nodo {
    ...
    /**
     * Parsea una condición del archivo (ej: "Rain=heavy;Maintenance=no;Train=delayed") a un mapa.
     */
    public static Map<String, String> parseAsignaciones(String condicion) {
        Map<String, String> map = new LinkedHashMap<>();
        if (condicion == null) return map;
    ...
    public double probabilidadDada(Map<String, String> asignacion) {
        ...
        for (Nodo p : padres) {
            if (!asignacion.containsKey(p.getNombre())) {
                throw new IllegalArgumentException("Falta valor para padre '" + p.getNombre() + "' al evaluar '" + nombre + "'");
            }
        }

        // Matching exacto: buscamos la fila cuya lista de asignaciones coincide para este nodo y sus padres.
        for (Map.Entry<String, Double> entry : tablaProbabilidad.entrySet()) {
            Map<String, String> fila = parseAsignaciones(entry.getKey());
            if (!myVal.equals(fila.get(nombre))) continue;
